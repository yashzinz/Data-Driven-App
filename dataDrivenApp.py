...
import random
import requests #for requesting from api
import webbrowser #for creating a hyperlink
...
def switch_frame(frame):
    frame.tkraise()
#function for messagebox for about button
def aboutPopup():
...
#function for messagebox for info button
def infoPopup():
...
#function for requesting data from potter api to use in the app
def getBook():
    url = f"https://api.potterdb.com/v1/books"  
    response = requests.get(url)
    data = response.json()
    #for generating a random integer in the range of (0-7) for generating a random title on click
    l_num = random.randint(0, len(data['data']))

    #for accessing the data from json file
    book_title = data['data'][l_num]['attributes']['title']
    #for replacing the empty labels with data from json file
    ...
    bookwiki_new = '\n'.join(wrap(bookWiki, width=40)) #for breaking the long link string
    book_wiki.configure(text=f": {bookwiki_new}")
    book_wiki.bind('<Button-1>', lambda x:webbrowser.open_new(f"\"{bookWiki}\"")) #for creating a hyperlink
    
    #for requesting images from json file
    ...
    if img_res.status_code == 200:
        ...
        cover_label.image = (
            img  #to prevent garbage collection
        )
...
#button on main frame, to give users an idea about the app
...
#button on main frame, to switch the frame to home frame
...
#button on home frame, to generate a book title
getbtn = CTkButton(home_frame, text=" Get a book " , command=getBook ,fg_color="#751807",  bg_color="#F59F14", text_color="#F59F14", hover_color="#b5250b", font=("Georgia", 20), width=150, height=50, corner_radius=0)
getbtn.place(x=220, y=527.2)

#button on home frame, to switch the frane to details frame
...
#empty label in home frame, to replace with title of the generated book
bookLabel = CTkLabel(home_frame, text="", font=("Georgia", 14, 'bold'), text_color="#751807", bg_color="#F59F14")
bookLabel.place(x=60, y=470)

#empty labels on details frame, to replace with details of the generated book
...
# back button for switching between frames
...
#info button for instructions on how to use the app
...
#for starting with main frame on launch
